...
export type RawCurrency = Omit<Currency, "id">

export const RAW_CURRENCIES: RawCurrency[] = [
  {
    currency: "BLUR",
...
const getData = (currency: RawCurrency[]): Currency[] => {
  const seenCurrencies = new Set()
  const newCurrencyData: RawCurrency[] = []

  currency.forEach((item) => {
    if (!seenCurrencies.has(item.currency)) {
      seenCurrencies.add(item.currency)
      newCurrencyData.push(item)
    }
  })
...
export const CURRENCIES = getData(RAW_CURRENCIES)
